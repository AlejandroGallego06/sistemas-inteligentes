...
def evalua(n,x,poblIt,utilidad,pesos):
    ...
    # Verificar si el peso supera el límite
    for i in range(n):
        if peso[i] > 15:
            fitness[i] = 0  # Si el peso excede el límite, establecer el fitness a cero
...
def seleccion(acumulado):
    ...
    if padre is None:
        # Si no se encontró ningún padre adecuado, elige uno al azar
        padre = poblIt[random.randint(0, n-1)]
